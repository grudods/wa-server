...
async function conectar() {
  ...
  sock.ev.on('connection.update', async (u) => {
    ...
    if (connection === 'close') {
      ...
      // Em ciclo de conflito, cada tentativa espera mais que a anterior,
      // ate 60s. Isso da tempo da outra instancia morrer de vez.
      let espera = 3000;
      if (substituida) {
        conflitosSeguidos++;
  ...
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    console.log(`📨 Chegou evento "${type}" com ${messages.length} mensagem(ns)`);

    if (type !== 'notify' && type !== 'append') {
      console.log(`   (ignorado: tipo "${type}" nao e mensagem nova)`);
      return;
    }
    ...
    for (const m of messages) {
      ...
      if (m.key.fromMe) {
        console.log(`   (ignorado: mensagem minha) ${jid}`);
        continue;
      }

      // Fora: grupos, canais/comunidades, status e transmissoes.
      if (jid.endsWith('@g.us') || jid.endsWith('@newsletter')
          || jid.endsWith('@broadcast') || jid === 'status@broadcast') {
        console.log(`   (ignorado: grupo/canal) ${jid}`);
        continue;
      }

      // O WhatsApp usa @s.whatsapp.net e, mais recentemente, @lid.
      // No @lid o endereco NAO e o telefone: o numero real vem em senderPn.
      let numero = null;

      if (jid.endsWith('@s.whatsapp.net')) {
        numero = jid.split('@')[0];
      } else if (jid.endsWith('@lid')) {
        const alternativo = m.key.senderPn || m.key.participantPn || '';
        numero = String(alternativo).split('@')[0] || null;
      }

      if (!numero || !/^[0-9]{6,15}$/.test(numero)) {
        console.log(`   (ignorado: endereco nao reconhecido) ${jid} | senderPn=${m.key.senderPn || '-'}`);
        continue;
      }
      ...
      if (!texto.trim()) {
        ...
        console.log(`   (ignorado: sem texto) ${numero} | tipo: ${tipos}`);
        continue;
      }
